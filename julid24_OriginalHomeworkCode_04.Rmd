...
# Scatterplot for longevity~brain size
# Referenced: https://ggplot2.tidyverse.org/reference/geom_text.html 
library(ggplot2)
p1 <- ggplot(kc, aes(Brain_Size_Species_Mean, MaxLongevity_m, label=rownames(kc)))
p1 <- p1 + geom_text(check_overlap=TRUE)
p1 <- p1 + geom_smooth(method="lm", formula = y~x, col="green")
p1
```
...
# Scatterplot for log(longevity)~log(brain size)
p2 <- ggplot(kc, aes(log(Brain_Size_Species_Mean), log(MaxLongevity_m), label=rownames(kc)))
p2 <- p2 + geom_text(check_overlap=TRUE)
p2 <- p2 + geom_smooth(method="lm", formula = y~x, col="red")
p2
```
...
slope_est1 <- coef(lb_model)[Brain_Size_Species_Mean]

# For log(longevity)~log(brain size)
slope_est2 <- coef(log_lb_model)[Brain_Size_Species_Mean]
slope_est2
```
